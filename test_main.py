...
def test_calculate_expenses_normal(client):
    ...
    assert [e["category"] for e in result["top3"]] == ["Rent", "Groceries", "Entertainment"]
...
@pytest.mark.parametrize("expenses,expected_top3", [
    ([{"category": "A", "amount": 100}, {"category": "B", "amount": 200}], ["B", "A"]),
...
@pytest.mark.parametrize("expenses", [
